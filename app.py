...
def index():
    ...
    stats = {
        ...
        'total': '1832',
        'rev_table' : df2.groupby(['App']).sum().sort_values(by = "Reviews", ascending=False).head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }

    ## Bar Plot
    cat_order = df2.groupby('Category').agg({
        'Category' : 'count'
        }).rename({'Category':'Total'}, axis=1).sort_values(by='Category').head()
    X = 'Category'
    Y = 'Total'
